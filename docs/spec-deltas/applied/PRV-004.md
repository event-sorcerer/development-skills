...
- **§6.11.1** WHEN `/peer-review` is invoked with a non-empty diff THE SYSTEM SHALL discover
  ...
  `visibility == "list"` AND `supported_in_api == true` AND a valid `slug` (string) and
  `priority` (integer) — `display_name`/`description`, if missing or non-string, default to
  the `slug` and an empty string respectively rather than excluding the model — and sorting
  the eligible result by `priority` ascending.
