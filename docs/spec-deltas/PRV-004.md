...
- **§6.11.2** THE SYSTEM SHALL present the discovered models to the human via
  `AskUserQuestion`, one option per model, and SHALL recommend (first-listed, labeled
  "Recommended") the lowest-`priority` eligible model — codex's own top-ranked pick. No other
  recommendation heuristic (e.g. diff-size-aware weighting) is used.
