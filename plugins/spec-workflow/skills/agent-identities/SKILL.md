...
**Consumer contract (peer-reviewer):** `plugins/peer-review/scripts/peer-review.sh` (PRV-002, merged) accepts an optional `--label <name>` flag / `PEER_REVIEW_LABEL` env var that overrides its default `"External review — codex"` findings-label string — the override exists precisely so a caller can supply the resolved `peer-reviewer` identity name instead, without `peer-review.sh` itself depending on the identity system (see `plugins/peer-review/README.md`). No caller wires this through yet: the `/peer-review` skill invocation (PRV-003) isn't built. Once it is, it should resolve `identity.sh peer-reviewer` and pass `--label "External review — <resolved name>"`.
