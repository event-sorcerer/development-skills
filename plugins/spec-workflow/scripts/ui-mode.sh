...
cfg_off() { [[ -f "$CONFIG" ]] && python3 -c 'import json,sys; sys.exit(0 if json.load(open(sys.argv[1])).get("methodology",{}).get("iterativeUI") is False else 1)' "$CONFIG" 2>/dev/null; }

case "${1:-status}" in
    status)
        if cfg_off; then echo "OFF (methodology.iterativeUI=false in .claude/project.json — project-wide)"
    ...
    on)
        rm -f "$FLAG"
        if cfg_off; then echo "flag removed, but still OFF: methodology.iterativeUI=false in project.json"
